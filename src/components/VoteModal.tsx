    ...
    <Dialog open={open} onClose={onClose} as="div" className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="bg-white rounded-lg shadow p-6 w-[90%] max-w-md">
          <h2 className="text-lg font-bold mb-4">{candidate.name}에게 투표하기</h2>
          ...
          {/* 연령 */}
          <div className="mb-2">
            <label className="block text-sm font-medium">연령대</label>
            <select value={age} onChange={(e) => setAge(e.target.value)} className="w-full border rounded px-2 py-1">
              <option>10대</option>
              <option>20대</option>
              <option>30대</option>
              <option>40대</option>
          ...
          <div className="mb-4">
            ...
            <select value={region} onChange={(e) => setRegion(e.target.value)} className="w-full border rounded px-2 py-1">
              <option>서울특별시</option>
              <option>부산광역시</option>
              <option>대구광역시</option>
              <option>광주광역시</option>
